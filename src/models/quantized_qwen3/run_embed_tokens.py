...
from torch2air.runtime.compile import compile_q4k_embedding_python_kernel
...
def prepare_inputs(
    *,
    ...
    for row_idx, token_id in enumerate(token_ids):
        ...
        raw_blocks[row_idx * blocks_per_row : (row_idx + 1) * blocks_per_row, :] = (
            np.frombuffer(payload, dtype=np.uint8).reshape(blocks_per_row, 144)
        )
    ...
    *,
    ...
    try:
        for _ in range(warmup):
            ...
            check_close_rocm(actual, expected, rtol=rtol, atol=atol, label="embed_tokens")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compile and run Python embed_tokens kernel.")
    parser.add_argument("--kernel-py", type=Path, required=True)
    parser.add_argument("--function-name", default="run_embed_tokens")
    ...
    if len(args.token_ids) != _kernel_sequence_length(args.kernel_py, args.function_name):
...
def _kernel_sequence_length(kernel_py: Path, function_name: str) -> int:
    builder = Q4KEmbeddingAirBuilder(function_name=function_name)
    load_kernel_function(kernel_py, function_name)(builder)
    if builder.embedding_output is None:
        raise RuntimeError("kernel did not emit aten.embedding.default")
