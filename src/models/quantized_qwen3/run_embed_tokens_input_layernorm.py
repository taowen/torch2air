    ...
    *,
    ...
    try:
        for _ in range(warmup):
            output.fill(0)
            actual = np.asarray(func(packed_rows, block_f16_scales, rms_weight, output)[3]).reshape(
                expected_shape
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run fused quantized_qwen3 embed_tokens + input_layernorm on real NPU."
    )
    ...
    parser.add_argument("--instance-name", default="run_embed_tokens_input_layernorm")
    ...
    packed_rows, block_f16_scales, rms_weight, expected, info = prepare_fused_inputs(
        gguf_path=args.gguf,
