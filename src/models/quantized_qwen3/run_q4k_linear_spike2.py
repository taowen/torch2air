...
from torch2air.export.q4k_linear_spike2 import (
    Q4K_LINEAR_SPIKE2_FUNCTION,
    Q4K_LINEAR_SPIKE2_HIDDEN_SIZE,
    Q4K_LINEAR_SPIKE2_LINK_OBJECT,
    Q4K_LINEAR_SPIKE2_OUTPUT_TILE_ROWS,
    build_q4k_linear_spike2_air,
)
...
def compile_q4k_linear_spike2_kernel(
    *,
    work_dir: Path,
    function_name: str,
    hidden_size: int,
    ...
    module = build_q4k_linear_spike2_air(
...
def compile_q4k_linear_spike2_object(
    *,
    work_dir: Path,
    ...
    object_file = work_dir / Q4K_LINEAR_SPIKE2_LINK_OBJECT
...
def prepare_q4k_linear_spike2_inputs(
    *,
    gguf_path: Path,
    token_ids: list[int],
    blocks_per_row: int,
    ...
    output_tile_rows: int,
    eps: float,
) -> tuple[np.ndarray, np.ndarray, torch.Tensor, Q4KLinearSpike2InputInfo]:
    if len(token_ids) != 1:
        raise ValueError("Spike 2 is decode-only and expects exactly one token")
    ...
    if blocks_per_row != model_blocks_per_row:
        raise ValueError(
            f"Spike 2 expects full rows with {model_blocks_per_row} Q4_K blocks, "
            f"got {blocks_per_row}"
        )
    if row_offset < 0 or row_offset + output_tile_rows > output_features:
        raise ValueError(
            f"row range [{row_offset}, {row_offset + output_tile_rows}) is outside "
            f"q_proj rows [0, {output_features})"
        )
    ...
    ).reshape(1, output_tile_rows)
    info = Q4KLinearSpike2InputInfo(
    ...
    *,
    ...
    try:
        for _ in range(warmup):
            ...
            check_close_rocm(actual, expected, rtol=rtol, atol=atol, label="q4k_linear_spike2")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Q4_K linear Spike 2 tile dot test.")
    parser.add_argument("--work-dir", type=Path, default=Path(".cache/npu-spikes/q4k-linear-spike2"))
    parser.add_argument("--function-name", default=Q4K_LINEAR_SPIKE2_FUNCTION)
    ...
    parser.add_argument("--hidden-size", type=int, default=Q4K_LINEAR_SPIKE2_HIDDEN_SIZE)
    ...
    print(f"row_offset {info.row_offset} output_tile_rows {info.output_tile_rows}")
