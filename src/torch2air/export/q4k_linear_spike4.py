...
Q4K_LINEAR_SPIKE4_FUNCTION = "q4k_linear_spike4"
Q4K_LINEAR_SPIKE4_HIDDEN_SIZE = 1024
Q4K_LINEAR_SPIKE4_OUTPUT_TILE_ROWS = 16
Q4K_LINEAR_SPIKE4_HERD_COLS = 2


def build_q4k_linear_spike4_air(
    *,
    function_name: str = Q4K_LINEAR_SPIKE4_FUNCTION,
    hidden_size: int = Q4K_LINEAR_SPIKE4_HIDDEN_SIZE,
    output_tile_rows: int = Q4K_LINEAR_SPIKE4_OUTPUT_TILE_ROWS,
    herd_cols: int = Q4K_LINEAR_SPIKE4_HERD_COLS,
) -> Module:
    if hidden_size <= 0:
    ...
    if herd_cols not in {2, 4}:
        raise ValueError(f"Spike 4 only tests 2 or 4 herd columns, got {herd_cols}")
    blocks_per_row = hidden_size // 256
    weight_words = blocks_per_row * 38
    output_rows = output_tile_rows * herd_cols
    ...
    def build() -> None:
        ...
        weight_l3_type = MemRefType.get([output_rows, weight_words], i32)
        output_l3_type = MemRefType.get([1, output_rows], f32)
