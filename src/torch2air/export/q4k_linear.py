...
class Q4KLinearAirBuilder:
    ...
    def herd_rows(self) -> int:
        ...
        if sequence_length != 1:
            raise ValueError("formal Q4_K linear currently compiles decode S=1 kernels")
        return 1

    def herd_cols(self) -> int:
        _, _, output_features = self.linear_shape()
    ...
    def linear_shape(self) -> tuple[int, int, int]:
        ...
        if sequence_length != 1:
            raise ValueError("formal Q4_K linear currently supports decode sequence_length=1")
...
def build_q4k_linear_air(
    *,
    ...
    herd_cols = output_features // output_tile_rows
