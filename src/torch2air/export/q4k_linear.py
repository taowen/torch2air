...
Q4K_LINEAR_TILE_FUNCTION = "q4k_linear_tile"
Q4K_LINEAR_LINK_OBJECT = "q4k_linear.o"
Q4K_LINEAR_PREFILL_BUCKET = 8

range_ = for_


class Q4KLinearAirBuilder:
    def __init__(
        self,
        *,
        function_name: str,
    ...
    def emit_kernel(
        ...
        *,
        ...
        if target != "aten.linear.default":
            raise ValueError(f"Q4_K linear builder only supports aten.linear.default, got {target}")
        if len(inputs) != 2:
            raise ValueError("Q4_K linear builder only supports bias-free aten.linear.default")
        if attrs:
    ...
    def herd_rows(self) -> int:
        ...
        if sequence_length == 1:
            return 1
        if sequence_length == Q4K_LINEAR_PREFILL_BUCKET:
            return 4
        raise ValueError(
            f"formal Q4_K linear supports S=1 or S={Q4K_LINEAR_PREFILL_BUCKET}, "
    ...
    def linear_shape(self) -> tuple[int, int, int]:
        ...
        if sequence_length not in {1, Q4K_LINEAR_PREFILL_BUCKET}:
            raise ValueError(
                f"formal Q4_K linear supports sequence_length=1 or "
                f"{Q4K_LINEAR_PREFILL_BUCKET}, got {sequence_length}"
            )
        ...
        if hidden_size % 256 != 0:
            raise ValueError(f"Q4_K input feature count must be divisible by 256, got {hidden_size}")
        ...
        if sequence_length == 1 and herd_cols > 4:
            raise ValueError(f"Q4_K linear herd columns must fit in 4 NPU columns, got {herd_cols}")
        if (
            sequence_length == Q4K_LINEAR_PREFILL_BUCKET
            and self.output_features != self.output_tile_rows
        ):
            raise ValueError(
                f"prefill Q4_K linear uses one output tile per xclbin; "
...
def build_q4k_linear_air(
    *,
    ...
    def build() -> None:
        ...
        tile_func = FuncOp(
            Q4K_LINEAR_TILE_FUNCTION,
            FunctionType.get([hidden_l1_type, weight_l1_type, output_l1_type], []),
            visibility="private",
        )
        tile_func.attributes["link_with"] = StringAttr.get(Q4K_LINEAR_LINK_OBJECT)
        ...
        def q4k_linear(hidden_l3: Value, weight_l3: Value, output_l3: Value) -> None:
