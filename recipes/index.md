...
| [official-demo-baseline.md](official-demo-baseline.md) | 出错时怎么用官方 demo 判断是环境问题还是自己 kernel 问题。 | 先跑同类最小官方 demo，再调自己的 ABI 和 tiling。 |
...
| [export-aten-sequence.md](export-aten-sequence.md) | 多个 aten op 的导出片段怎么接 AIR。 | 保留 export op 顺序，逐 op 映射到本地 emitter。 |
...
| [host-stage-boundary.md](host-stage-boundary.md) | stage 间中间 BO 必须 host-visible 时怎么切边界。 | 拆成多个 host launch，每个 stage 一个 xclbin/insts。 |
| [runtime-sequence-output-drain.md](runtime-sequence-output-drain.md) | single herd 有多个 public 写回 BO 时为什么只写前几个 tile。 | host-visible 写回 channel 不一定按源码顺序交错 drain。 |
| [multi-launch-pdi-packaging.md](multi-launch-pdi-packaging.md) | multi-launch 生成多个 segment/PDI 时为什么当前 xclbin 不可靠。 | `--xclbin-input` 和手工 JSON merge 都不是同 kernel multi-segment 修复。 |
| [rope-prefill-row-tile.md](rope-prefill-row-tile.md) | RoPE/RMSNorm prefill 多 token 怎么避免残留 `air.wait_all` 和只写前几个 head。 | prefill 按 token row 切 herd，external kernel 在 L1 内循环 heads。 |
